...
router.post("/", (request, response) => {
  const newNeighborhood = new Neighborhood(request.body);
  newNeighborhood.save((error, record) => {
...
router.get("/", (request, response) => {
  Neighborhood.find({}, (error, record) => {
    if (error) return response.status(500).json(error.errors);
...
router.get("/:id", (request, response) => {
  Neighborhood.findById(request.params.id, (error, record) => {
    if (error) return response.status(500).json(error.errors);
...
router.delete("/:id", (request, response) => {
  Neighborhood.findByIdAndRemove(request.params.id, {}, (error, record) => {
    if (error) return response.status(500).json(error.errors);
